...
L'obiettivo è stimare l'effetto medio del trattamento (in inglese ATE, [Average treatment effect]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"} ):
...
-   La popolazione trattata(o di trattamento) ,$Y_i(1)$ , risulta avere un età più alta e una visione politica più di [*destra/conservatrice*]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"}

-   Mentre la popolazione non trattata (o di controllo), $Y_i(1)$ , risulta essere più giovane e con una visione politica più di [*sinistra/liberale*]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"}
...
Sotto questa ipotesi, la differenza tra la media degli outcomes nel gruppo di trattamento e la media degli outcomes nel gruppo di controllo fornisce una stima [non distorta]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"} dell'effetto medio del trattamento (ATE).

In contesti osservazionali, invece, sono necessari metodi più avanzati come l'IPW e l'AIPW.

### Stima di $\tau$ in un contesto [sperimentale]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"}
...
e calcolando la media nel gruppo di [controllo]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"}
...
## 3. [Direct]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"} estimator
...
## 4. [IPW]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"} (Inverse propensity-weighted estimator)
...
ipw_estimator <- function (data,
  ...
  w <- data[,treatment_variable]
  fmla <- as.formula(
    paste0("~", paste0(covariates, collapse="+"))
  )
  XX <- model.matrix(fmla,data)
  logit <- cv.glmnet(x=XX, y=w, family="binomial") # fitting a logistic model on treatment variable
...
```{r}
ipw_estimator(data_real,outcome,treatment,covariates)
```

## 5. [AIPW]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"} (Augmented inverse propensity-weighted estimator)
...
XX <- model.matrix(formula(paste0("~", paste0(covariates, collapse="+"))), data=data_real)

# Estimate a causal forest.
forest <- causal_forest(
              X=XX,  
              W=data_real[,treatment],
              Y=data_real[,outcome],
...
## 6. Elementi di [diagnostica]{style="background: linear-gradient(180deg,rgba(255,255,255,0) 50%, #0184a0 50%);"}
...
balancer <- function (propensity_scores,
                      treatment,
                      covariates,
                      data) {
...
df <- data.frame(
  ...
  Unadjusted_SMD = abs.mean.diff / std,
  Adjusted_SMD = abs.mean.diff.adj / std.adj
)

# Gather the data into a long format
df_long <- df %>%
  pivot_longer(cols = c(Unadjusted_SMD, Adjusted_SMD), names_to = "Adjustment", values_to = "SMD")

# Create the faceted plot
ggplot(df_long, aes(x = SMD, y = factor(Covariate))) +
  geom_point(size = 3,color="white") +
  facet_wrap(~ Adjustment, ncol = 2) +
  geom_vline(xintercept = seq(0, 1, by = 0.25), linetype = "dashed", color = "grey50", linewidth = 0.5) +
  geom_hline(yintercept = seq_len(nrow(df)), linetype = "dashed", color = "grey50", linewidth = 0.5) +
  scale_x_continuous(limits = c(-.01, 1.01), expand = c(0, 0)) +
  scale_y_discrete() +
  labs(x = "Standardized Mean Difference", y = NULL, title = "Covariate Balance") +
  theme(
    # Set black background
    plot.background = element_rect(fill = "#242728"),
    panel.background = element_rect(fill = "#242728"),
    ...
    # Optional: adjust grid lines for better visibility
    panel.grid.major = element_line(color = "#242728"),
    panel.grid.minor = element_line(color = "#242728"),
...
balancer(propensity_scores = forest$W.hat,
         treatment = treatment,
