...
**L'inferenza causale** è fondamentale perché ci permette di comprendere le relazioni tra cause ed effetti nei fenomeni che osserviamo. A differenza dell'analisi della semplice associazione tra variabili, *l'inferenza causale* mira a stabilire relazioni causali, ovvero a identificare l'effetto di una variabile (il [trattamento]{.double-underline}) sulla variabile di interesse ([l'outcome]{.double-underline}), tenendo conto di tutte le possibili *covariate*.
...
## 6. [Elementi di diagnostica]{.circled}
