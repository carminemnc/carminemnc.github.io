...
```{mermaid}
%%| label: fig-pipeline
%%| fig-cap: "Data pipeline"
flowchart LR
    A(Data extraction) --> B{Pre-processing}
    B --> C(Bigrams analysis)
    B --> D(Word trends analysis)
...
::: {.callout-tip collapse="true" title="A further interpretation" icon="false"}
Note that this is a visualization of a **Markov chain**, a common model in text processing. In a Markov chain, each choice of word depends only on the previous word. So in this case the chain starting from **"hotel"** would suggest **"friendly, staff"** as next words, by following each word to the most common words that follow it.   ^[https://www.tidytextmining.com/ngrams]
:::
